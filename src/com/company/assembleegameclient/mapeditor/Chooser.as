...
public class Chooser extends Sprite
   {
      ...
      public static const HEIGHT:int = 396;
      ...
      private var currentElements_:Vector.<Element>;
      ...
      function Chooser(layer:int)
      {
         ...
         this.currentElements_ = new Vector.<Element>();
         super();
         this.layer_ = layer;
         this.drawBackground();
      ...
      public function reloadElements(filter:String) : void
      {
         this.currentElements_.length = 0;
         this.elementSprite_.removeChildren();

         var element:Element;
         for each (element in this.elements_)
            if (element.name_.toUpperCase().indexOf(filter.toUpperCase()) >= 0)
      ...
      protected function addElement(element:Element, init:Boolean = true) : void
      {
         ...
         this.currentElements_.push(element);
         if (init)
